...
begin
  if ActiveRecord::Base.connection.table_exists?(:plans)
    plans = [
      ...
      { key: :starter_yearly,     name: "Starter (Yearly)",    env_price_key: "STRIPE_PRICE_STARTER_YEARLY",      interval: "year", price_cents: 1900, active: true, position: 4 },
      { key: :pro_yearly,         name: "Pro (Yearly)",        env_price_key: "STRIPE_PRICE_PRO_YEARLY",          interval: "year", price_cents: 6900, active: true, position: 5 },
      { key: :enterprise_yearly,  name: "Enterprise (Yearly)", env_price_key: "STRIPE_PRICE_ENTERPRISE_YEARLY",  interval: "year", price_cents: 19900, active: true, position: 6 }
    ]
